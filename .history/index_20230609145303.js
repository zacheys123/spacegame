	...
	shoot(invproj) {
		...
		invproj.push(
			new InvaderProjectile({
				...
				velocity: { x: 0, y: val + Math.random() * val * 0.1 },
			}),
		);
	}
}
		...
		projectiles.forEach((projectile, id) => {
			if (
				...
				projectile.position.y - projectile.radius <=
					invaderprojectile.position.y + invaderprojectile.height && // check that the bottom part of the projectile is below the invader
