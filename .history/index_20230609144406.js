	...
	shoot(invproj) {
		...
		invproj.push(
			new InvaderProjectile({
				...
				velocity: { x: 0, y: val + Math.random() * val * 3 },
			}),
		);
	}
}
		...
		projectiles.forEach((projectile, id) => {
			if (
				invaderprojectile.position.y + invaderprojectile.height >=
					canvas.height && //checks that the projectiles top part is below the invader top part
				// top of projectile
				//
				projectile.position.y - projectile.radius >=
					invaderprojectile.position.y + invaderprojectile.height &&
				//
				// check that the bottom part of the projectile is below the invaderprojectile
				projectile.position.x + projectile.radius >=
					// invaderprojectile bottom part	//
